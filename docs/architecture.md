...
- **Layered architecture**:
  ...
  - **Service** — `src/services/` (business rules; e.g. `roomService`, `participantService`, `roomEventService`, `queueItemService`, `voteService`, `skipVoteService`, `volumeVoteService`)
...
- **Skip vote API**: `skipVoteState(roomId, participantId?)` → `{ queueItemId, voteCount, participantCount, threshold, passed, viewerHasVoted }`; `toggleSkipVote(participantId)` toggles skip for current now-playing (touches activity); `clearNowPlaying(roomId)` when the desk goes idle (also clears volume votes); subscription `skipVoteStateUpdated(roomId)`. Quorum = simple majority of **active guests** (`floor(n/2)+1`) where active means `lastActiveAt` within **20 minutes**. Skip `voteCount` only includes votes from currently active guests. Host Now Playing shows active count + `voteCount/threshold`. Participant view touches activity on mount and on interaction (throttled); heartbeats for guests already inside the TTL do not republish skip/volume state. **Inactivity notify**: `activitySweepService` finds guests whose `lastActiveAt` just fell outside the TTL, posts `BECAME_INACTIVE` room events (deduped), and republishes skip + volume state (~1m in-process loop on Fluid; Vercel Cron daily on Hobby).
- **Volume vote API**: `volumeVoteState(roomId, participantId?)` → `{ upCount, downCount, netCount, participantCount, threshold, passed, direction, viewerVote }`; `setVolumeVote(participantId, value: UP|DOWN|NONE)`; `acknowledgeVolumeNudge(roomId)` (host clears votes after applying a nudge); subscription `volumeVoteStateUpdated(roomId)`. Quorum independent of skip: `floor(n/3)+1` of active guests; `passed` when `|net| >= threshold` (`net = up − down`). Interim nudge step: **10%** of the 0–100 volume scale (host-local). Per-room quorum/nudge settings are future work.
