...
- **UI layers** (atomic-style):
  ...
  - `src/features/` — product behaviors (create-room, room-detail, room-settings, …)
...
- **Host desk layout** (`views/host-room` + `features/host-desk`):
  ...
  - **Player** — provider-agnostic `MediaPlayer` (`features/player/`; `setVolume`/`getVolume` 0–100); `YouTubeMediaPlayer` wraps the YouTube IFrame Player API (no API key required); factory `createMediaPlayer(type)`. Playlist tiles show persisted `title` + `thumbnailUrl` from the API. Host desk soft-pops (`popQueueItem`) as soon as an item starts playing so votes cannot reshuffle the active track; on embed `ENDED`, clears local now-playing and advances to the next queue head the same way. Soft-pop also sets `Room.nowPlayingQueueItemId` and resets skip + volume votes (broadcast via `skipVoteStateUpdated` / `volumeVoteStateUpdated`). Host Now Playing header shows **Active** listener count, skip tally `voteCount/threshold`, and a volume-vote bipolar meter (net vs ±threshold) plus host-local volume `%`; when skip quorum passes, host advances like a natural end; when volume quorum passes, host nudges volume by `VOLUME_NUDGE_PERCENT` (10) and calls `acknowledgeVolumeNudge` to clear volume votes for another round. Volume level is host-local only (not persisted / not broadcast to guests).
  - **Participant room** — now-playing thumbnail (`Room.nowPlaying`, live via `queueItemPopped` / idle clear) at the top + YouTube URL/id submit form (`addQueueItem`) + live horizontal queue list of all upcoming room tracks with per-item up/down vote toggles (`voteOnQueueItem`) + skip-current toggle (`toggleSkipVote`) + volume three-way controls (`setVolumeVote` UP|DOWN|NONE); both skip and volume votes reset when the next track soft-pops; participant-view interactions call `touchParticipantActivity` (20m active TTL for skip/volume quorum)
- **GraphQL client**: Apollo Client → `VITE_GRAPHQL_URL` (default `/api/graphql`)
  ...
  - `touchParticipantActivity` heartbeats for already-active guests do not republish `skipVoteStateUpdated` / `volumeVoteStateUpdated` (only membership changes in the active set do)
  - Guests aging out of the 20m TTL are detected by an activity sweep (~1m in-process interval on long-lived / Fluid instances). Vercel Cron `GET /api/internal/sweep-inactive` is a Hobby-safe daily safety net (`0 0 * * *`; Pro can use `* * * * *`); matching guests get a `BECAME_INACTIVE` room event and their rooms get a fresh `skipVoteStateUpdated` + `volumeVoteStateUpdated` so Active counts stay current
...
- **Layered architecture**:
  ...
  - **Service** — `src/services/` (business rules; e.g. `roomService`, `participantService`, `roomEventService`, `queueItemService`, `voteService`, `skipVoteService`, `volumeVoteService`; room settings live on `Room` via `roomService.updateSettings`)
...
- **HTTP routes**: `/api/health` (REST health); `/api/internal/sweep-inactive` (cron; inactivity → skip/volume pub/sub); GraphQL at `POST /api/graphql` + `WS /api/graphql` (subscriptions).
- **Domain**:
  - `Room` — `id` (Mongo), `shortId` (5-char join code), `name`, optional `nowPlayingQueueItemId` (soft-popped item on the host desk), settings (`skipQuorumPercent` default 51, `volumeQuorumPercent` default 34, nullable `maxSubmissionDurationMinutes` / `maxSimultaneousSubmissions`), timestamps; `room(id)` accepts shortId or ObjectId; `participants` field lists members; `events` lists `RoomEvent`s chronologically; `queueItems` lists active `QueueItem`s by vote score; `nowPlaying` resolves the soft-popped `QueueItem` (or null when idle)
  - `Participant` — `id`, `roomId`, `role` (`HOST` | `GUEST`), optional `name`/`nameKey` for guests only (unique per room, case-insensitive); hosts are unnamed desk operators; guests track `lastActiveAt` for skip/volume quorum
  ...
  - `VolumeVote` — `id`, `roomId`, `queueItemId`, `participantId`, `value` (`UP` | `DOWN`), timestamps; unique per `(roomId, participantId)`; cleared on soft-pop / idle clear / `acknowledgeVolumeNudge`
  - Embed resolution — `src/lib/mediaEmbed.ts` parses provider media refs (YouTube URL/id) and builds privacy-enhanced embed URLs
  - Media metadata — `src/lib/mediaMetadata.ts` (`MediaMetadataProvider`); YouTube implementation calls Data API `videos.list` (`part=snippet,contentDetails`) using server env `YOUTUBE_API_KEY` (title, thumbnail, duration)
- **Room shortId**: Ambiguity-safe alphabet (`A–Z` / `2–9`, no `0/O/1/I/L`); unique; used in `/rooms/:roomId` URLs and join form.
- **Participant API**: `createRoom(name)` → `{ room, participant }` (unnamed host); `joinRoom(roomId, name)` → named guest (sets `lastActiveAt`); `leaveRoom(participantId)` (clears that guest’s skip + volume votes); `touchParticipantActivity(participantId)` refreshes `lastActiveAt`; `participant(id)`
- **Room settings API**: `updateRoomSettings(participantId, input)` (host-only) updates `skipQuorumPercent` / `volumeQuorumPercent` (1–100) and optional caps `maxSubmissionDurationMinutes` / `maxSimultaneousSubmissions` (null = disabled); republishes skip + volume state so live thresholds refresh. Host desk Settings modal edits these.
...
- **QueueItem API**: `queueItems(roomId)` / `Room.queueItems` (active/`finished: false` only, sorted by `score` desc then `createdAt` asc); `Room.nowPlaying` (soft-popped item or null); `addQueueItem(participantId, type, mediaRef)` (touches activity, posts `ITEM_SUBMITTED`; rejects when duration exceeds `maxSubmissionDurationMinutes` or the guest already has `maxSimultaneousSubmissions` active items); `popQueueItem(id)` marks `finished: true`, sets `Room.nowPlayingQueueItemId`, clears skip + volume votes, posts `NOW_PLAYING`, publishes `queueItemPopped` + `skipVoteStateUpdated` + `volumeVoteStateUpdated`; field `viewerVote(participantId)` on `QueueItem`; subscriptions `queueItemAdded(roomId)`, `queueItemPopped(roomId)`, `queueItemUpdated(roomId)`
- **Vote API**: `myQueueVotes(roomId, participantId)`; `voteOnQueueItem(participantId, queueItemId, value)` → `{ queueItem, value }` — upserts one vote per participant per item; submitting the same value again clears the vote; adjusts denormalized `score`, touches activity, and publishes `queueItemUpdated` + `skipVoteStateUpdated` + `volumeVoteStateUpdated`
- **Skip vote API**: `skipVoteState(roomId, participantId?)` → `{ queueItemId, voteCount, participantCount, threshold, passed, viewerHasVoted }`; `toggleSkipVote(participantId)` toggles skip for current now-playing (touches activity); `clearNowPlaying(roomId)` when the desk goes idle (also clears volume votes); subscription `skipVoteStateUpdated(roomId)`. Quorum = `ceil(n * skipQuorumPercent / 100)` of **active guests** (default percent **51**, matching former simple majority) where active means `lastActiveAt` within **20 minutes**. Skip `voteCount` only includes votes from currently active guests. Host Now Playing shows active count + `voteCount/threshold`. Participant view touches activity on mount and on interaction (throttled); heartbeats for guests already inside the TTL do not republish skip/volume state. **Inactivity notify**: `activitySweepService` finds guests whose `lastActiveAt` just fell outside the TTL, posts `BECAME_INACTIVE` room events (deduped), and republishes skip + volume state (~1m in-process loop on Fluid; Vercel Cron daily on Hobby).
- **Volume vote API**: `volumeVoteState(roomId, participantId?)` → `{ upCount, downCount, netCount, participantCount, threshold, passed, direction, viewerVote }`; `setVolumeVote(participantId, value: UP|DOWN|NONE)`; `acknowledgeVolumeNudge(roomId)` (host clears votes after applying a nudge); subscription `volumeVoteStateUpdated(roomId)`. Quorum independent of skip: `ceil(n * volumeQuorumPercent / 100)` of active guests (default percent **34**, matching former `floor(n/3)+1`); `passed` when `|net| >= threshold` (`net = up − down`). Interim nudge step: **10%** of the 0–100 volume scale (host-local). Volume level is host-local only (not persisted / not broadcast to guests).
- **HTTP internal**: `GET|POST /api/internal/sweep-inactive` — authorized with `Authorization: Bearer $CRON_SECRET`; used by Vercel Cron (`0 0 * * *` in root `vercel.json` — Hobby allows at most one run/day; use `* * * * *` on Pro)
- **Realtime**: TypeGraphQL + `@graphql-yoga/subscription`; Redis via `@graphql-yoga/redis-event-target` when `REDIS_URL` is set, otherwise in-memory pub/sub (single process / tests). Topics: `ROOM_EVENT`, `QUEUE_ITEM_ADDED`, `QUEUE_ITEM_POPPED`, `QUEUE_ITEM_UPDATED`, `SKIP_VOTE_STATE`, `VOLUME_VOTE_STATE`. Transport: `graphql-ws` over WebSockets.
...
- **Tests**: Jest + Supertest against `createApp()`; GraphQL Room/Participant/RoomEvent/QueueItem/SkipVote/VolumeVote tests use `mongodb-memory-server`.
...
- **Serverless entry**: `api/index.ts` exports an `http.Server` from `createHttpServer()` (Express + Apollo HTTP + `graphql-ws` WebSockets). Matches [Vercel Functions WebSockets](https://vercel.com/docs/functions/websockets) (native Node `ws`, not `experimental_upgradeWebSocket`).
  ...
  - Activity sweep loop also starts inside long-lived Function instances (daily cron is a cold-period safety net on Hobby)
