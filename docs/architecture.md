...
- **GraphQL client**: Apollo Client → `VITE_GRAPHQL_URL` (default `/api/graphql`)
  ...
  - Guests aging out of the 20m TTL are detected by an activity sweep (~1m in-process interval on long-lived / Fluid instances). Vercel Cron `GET /api/internal/sweep-inactive` is a Hobby-safe daily safety net (`0 0 * * *`; Pro can use `* * * * *`); matching guests get a `BECAME_INACTIVE` room event and their rooms get a fresh `skipVoteStateUpdated` so Active counts stay current
